...
class Category extends Component {
  ...
  addCategory = async () => {
    let {addCategoryData} = this.state;
    if(addCategoryData.name.trim().length>0){
      let res = await reqAddCategory(addCategoryData.name,addCategoryData.parentId);
      if(res.code=='0'){
        //隐藏弹窗
  ...
  render() {
    ...
    const extra = (<Button type="primary" onClick={this.showModal}><Icon type='plus'></Icon>添加</Button>);
    return (
        <div>
